...
- **1인 1토큰**: 멤버당 최대 1개의 토큰만 보유한다. 재갱신 시 기존 토큰을 교체한다.
  (`userId @unique` 로 DB 레벨에서 보장) — 이는 도메인 문서가 강제한 요구는 아니며,
  본 spec 레벨의 설계 결정이다(문서의 "재갱신" 문구와 자연스럽게 맞물림).
...
따라 다르게 표시한다. 동시 클릭(발급 버튼 중복 클릭)은 `userId @unique` + `upsert`
조합으로 자연히 안전하다(최종적으로 1개 row 유지).
