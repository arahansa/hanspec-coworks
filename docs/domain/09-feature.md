...
- **TAG**: 프로젝트 스코프 `Tag` 모델 + `NodeTag` 조인(다대다). 자동완성 펼침목록은 프로젝트별로 모은다. FEATURE·REQUIREMENT에 부여(2026-06-04: REQUIREMENT 확장, 05-tag.md). `@`로 하나씩 입력하며, 없는 태그는 신규 생성·있는 태그는 목록에서 선택.
